...
SELECTION_TIME_COLS: frozenset[str] = frozenset({
    ...
    # (2026-08-04 정리) 여기 있던 'NXT 19:25 야간 갭' 주석은 **실재하지 않는 컬럼**을 설명하고
    # 있었다 — 그 축은 sql 마이그레이션도 컬럼도 만들어지지 않았고(daily_stock_report 에 1925
    # 계열 컬럼 없음), 참조하던 `sql/47` 번호는 다른 시드가 쓰고 있다. 19:50 갭(`nxt_gap_pct`)은
    # 아래 EXECUTION_TIME_COLS 에서 집행 레이어로 다룬다.
...
def check_promotion(rule: dict, control_rules: list[dict], policy: str = "strict") -> dict:
    ...
    if role == "selector":
        ...
        # 대칭성상 **약 50%**다(당시 남아 있던 월 승격 상한보다도 문턱이 낮아졌다).
        # 그 상태에서는 과적합 방어가 월 상한 하나뿐이므로, 상한에 의존하는 대신
        ...
        # ① mean_net>0(비용 차감 후 실제 수익) ② 거래일≥10 ③ ci_low>0 ④ 강등 감시
        # (check_demotion, 최근 창) ⑤ 승격 시 관리자 수동 승인. 초과수익은 계속 계산되어 룰 상세
        # 화면에 뜨므로 **승인 전 그 값을 눈으로 확인하는 것이 이 방어선의 일부**다.
        # (2026-08-05: 여기 있던 '월 승격 상한'은 폐지됐다 — routers/edge_rule 상단 주석 참조.
        #  통과 건수를 사람이 줄이는 장치가 사라졌으므로 위 게이트가 방어의 전부다.)
