...
# 괄호 안은 뉴스 발행처/말머리/관례어라 종목이 아니다. 매칭 전 제거한다.
#   [] 【] — 발행처·말머리([한국경제]·[속보]·[단독]). 발행처명이 상장사명과 겹치는 오탐 차단.
#   ()   — 기사 말미 관례어((상보)=상세보도·(종합)·(1보)). 2026-08-05 추가:
#          `7월 소비자물가 2.8%↑(상보)`·`코스닥 매수 사이드카(상보)` 가 전부 상보(027580)로
#          잡히고 있었다. 14일 실측 70건이 사라지고 그중 67건이 상보다 — **정상 매칭 손실 0건**
#          (나머지 3건도 괄호 안 발행처·규격 표기라 전부 오탐이었다).
