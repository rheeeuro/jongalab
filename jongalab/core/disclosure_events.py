...
# 설계 초기 가정("공시는 stock_code 로 당사자가 확정된다")은 틀렸다: 지배회사가 자회사
# 사건을 자기 코드로 접수한다. 실제 오탐(2026-07-28 감사) —
#   고려아연/신한알파리츠 '유상증자결정(종속회사의주요경영사항)'  → 모회사 희석 아님
#   골프존홀딩스 '출자법인의회생절차및파산절차관련사실등발생'      → 본인 회생 아님
#   코오롱 '단일판매ㆍ공급계약해지(자회사의 주요경영사항)'        → 본인 계약 아님
...
_RULES: tuple[tuple[re.Pattern, str, int, int], ...] = (
    ...
    # 이미 소화한 악재로 뒤늦게 제외하게 된다(2026-07-28 실수집 오탐 감사에서 발견).
    ...
    # 아래 무상증자(호재 +1)·유상증자 룰에 먼저 걸려 방향이 거꾸로 붙었다
    # (2026-08-04 알테오젠 실수집: 권리락 공시가 direction=+1 호재로 저장됨).
    (re.compile(r"권리락"), "권리락", 0, 0),
    # ── 존속 위험 (veto) — 실질 위험 사유만. 거래정지 전반을 잡으면 안 된다 ──
    # 거래정지의 대다수는 액면병합·주식분할·무상증자·SPAC합병 같은 **기술적 절차**이고,
    # '거래정지해제'는 오히려 정상화 신호다(2026-07-28 감사: 상장위험 44건 중 대부분 오탐).
    ...
    # (2026-07-28 감사: 풍문 매칭 11건 중 대형주 해명이 다수). 관측만 하고 제외하지 않는다.
    ...
    #   2026-07-27 NAVER 가 NVIDIA 대상 제3자배정 유상증자(1.48조, 희석 4.6%)를 냈고 주가가
    #   올랐다. 제목 기반 분류는 이를 악재로 보고 그날 **1순위 종목**을 제외할 뻔했다.
