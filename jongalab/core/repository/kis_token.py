"""한국투자증권(KIS) 액세스 토큰 데이터 접근 (단일행, id=1).

kiwoom_token 리포지토리와 동형이나, jongalab DB(core.db.get_db)에 보관한다.
"""
...
def save_token(access_token: str, expires_dt: Optional[str]) -> None:
    ...
    with get_db() as (conn, cursor):
        cursor.execute(
            """INSERT INTO kis_token (id, access_token, expires_dt)
               VALUES (1, %s, %s)
...
def clear_token() -> None:
    """토큰 행 삭제"""
    with get_db() as (conn, cursor):
        cursor.execute("DELETE FROM kis_token WHERE id = 1")
        conn.commit()
