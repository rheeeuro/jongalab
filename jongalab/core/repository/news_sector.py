...
def get_unlabeled_headlines(limit: int, since: datetime | None = None,
    ...
    `since`(+`newest_first=True`)는 **그날 00시 이후를 최신부터** — 매수 판단 전에 도는 실행용이다.
    백로그 소화가 목적인 야간 실행과 목적이 다르다: 그쪽은 표본의 연속성이, 이쪽은 '오늘 나온
    기사가 매수 시점에 라벨을 갖고 있는가'가 중요하다. 범위를 오늘로 좁히는 건 밀린 백로그와
    상한을 두고 경쟁하지 않게 하기 위해서다. 두 모드가 한 워커에 있는 이유는 판정·적재 경로를
    하나로 유지해야 라벨 정의가 갈라지지 않기 때문이다.
    """
    params: list = []
    where = ""
