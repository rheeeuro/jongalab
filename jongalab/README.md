...
종가 분석(closing_bet, 평일 13:00~15:00)│
  ...
  종합점수 = 수급 + 정배열 + 신고가 + 대장주 + 테마(거래대금 상위 50 교집합만) + 콘텐츠 + 뉴스 + 등락률(2~12% 가점 / 15%+ 감점) (가중치 튜닝 대상)
        ...
        │       분기 저속 데이터라 매일 중복 저장(연구용 무해), 부채비율은 ka10001 미제공이라 제외. rule 은 의도적으로 미등록(데이터부터 축적) —
        │       veto(적자·극단 고PER 제외)·품질 필터 축을 표본 축적 후 판정해 등록. 선정 시점 컬럼이라 SELECTION_TIME_COLS 포함(향후 live 자격)
