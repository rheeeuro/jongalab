...
| `news_matcher.py` | 뉴스 헤드라인 → 종목 사전매칭(LLM 없음). ticker_dictionary(ACTIVE) 인메모리 매처, 경계 룩어라운드 + 발행처 대괄호([]·【】) 제거로 오탐 억제. **텔레그램 경로 전용**(네이버는 종목코드 조회라 이 매처를 통과하지 않는다). 실패분(`content_skip` reason=`no_match`)이 55%지만 2026-07-31 코퍼스 45건 육안 분류 결과 **상장사 귀속이 가능한 건 9%뿐**이었다(나머지는 해외·정치·연예·지역·거시 — 애초에 종목 뉴스가 아니다). 그 9% 중 절반은 사명이 문자열로 들어있는데 우측 경계에 걸린 케이스다("IBK기업은행"·"기업은행**장**" 안의 `기업은행`) → 회수는 별칭 사전이나 좁은 접미사 예외로 하고 **LLM 귀속으로 대체하지 않는다**: 실효 이득이 4~5%인데 ① 사실 층이 LLM 의존이 되어 OpenAI 장애 시 원자료가 0이 되고(그 데이터를 자금 경로 `news_guard` 가 읽는다) ② 결정론이 깨져 `news_prior_avg` 기저의 소급 재계산·감사가 불가능해지고 ③ 남은 90%에 들이대면 테마 추론(감)이 사실 라벨에 섞인다 |
| `naver_news_client.py` | **네이버 증권 종목별 뉴스 클라이언트** — `GET {BASE}/api/news/stock/{code}?pageSize=` JSON 을 감싼 최소 래퍼(인증 없음, 브라우저 UA 필요). 응답은 '관련기사 묶음' 배열이라 `items` 를 평탄화하고, `officeId`/`articleId`(안정 URL = `(source_url, ticker)` 멱등키) · `officeName`(언론사) · `datetime`(YYYYMMDDHHMM **분 단위**) · `titleFull`(목록용 `title` 은 잘려 온다) 만 쓴다. HTML 엔티티(`&quot;` 등) 치환은 여기서 한 번. **반환 계약 고정**(`fetch_stock_news(code)` → `[{headline, source_url, channel_name, published_at}]`)이라 소스를 갈아끼워도 워커는 무변경 — 403/429 는 `NaverNewsBlocked` 로 따로 올려 호출부가 사이클을 조기 종료한다(막힌 상태로 계속 두드리면 차단이 길어진다). `body` 발췌도 응답에 오지만 저장하지 않는다(헤드라인 dedup 키가 흔들린다 — 승격 후 재검토) |
...
`content`(콘텐츠 분석) · `news`(뉴스 속보 언급 `news_mention` — **소스 2개가 한 테이블에** 들어온다: 텔레그램 채널(사명 매칭)과 네이버 증권 종목별 뉴스(`source='naver'`, 매칭 없음). ⚠️ **조회 함수는 전부 `_source_filter()` 를 통과한다** — `config.NEWS_ACTIVE_SOURCES`(기본 `telegram`)에 없는 소스는 적재만 되고 라벨·룰·veto·LLM 판정에서 완전히 빠진다. 왜: 소스를 늘리면 `news_count`·`news_prior_avg`·surprise 배수가 도입일에 계단식으로 튀는데(실측 2026-07-31 네이버 784행 vs 텔레그램 280행, 헤드라인 중복 2%뿐이라 상쇄 없이 순증) `news_unique_count` 를 predicate 에 쓰는 rule 이 3종이고 그중 `veto_bad_news` 가 **live = 자금 경로**다. 새 조회 함수를 추가할 때 이 필터를 빠뜨리면 게이트가 조용히 뚫린다. `save_news_mentions` 는 `source`·`created_at` 을 행별로 받고 미지정 시 `telegram`·지금 시각) · `stock_event`(**사건 계층** — 종목×사건 1행 정규화, **영구 보존**. DART 공시 멱등 적재(`save_events`, `(source, source_key=접수번호)` UNIQUE)·일자별 벌크 조회(`get_events_by_date`)) · `source`(채널) · `ticker`(기업명↔티커, 상장종목 벌크 시딩) · `stock_report`(종목일간리포트 — 리포트 저장·갭 체크·NXT 스냅샷·결과 백필) ·
...
| ⏰ `naver_news_collector` | 평일 08:45~20:45 (30분, :15/:45) | **네이버 증권 뉴스 수집** — 대상 종목(오늘 유니버스 ∪ 전거래일 유니버스 ∪ 보유 종목) 별로 `fetch_stock_news` 1페이지 조회 → **당일 기사만** → `news_mention` 에 `INSERT IGNORE`(`(source_url, ticker)` UNIQUE → 재실행 멱등, 실측 재실행 신규 0건). **왜**: 유니버스 종목 중 뉴스 라벨을 받는 건 47%뿐이었다(2026-07-31 실측 18/38) — 남은 53%는 텔레그램 채널이 그 종목 기사를 안 옮겨서 생긴 공백이고 `no_match` 개선으로는 안 메워진다(귀속 가능분 9%). 네이버는 **종목코드로 조회하므로 사명 매칭이 없다** → 실측 커버리지 90%(57/63종목·784건·27초). **`created_at` 에 기사 발행시각을 넣는다**(수집 시각이 아니다) — `news_pm_count`(12시 이후)·`get_news_heat`(NOW() 기준 창)·`news_guard` 오버나잇 창의 기준이라, 30분 주기 수집기가 수집 시각을 쓰면 저녁 사이클 분이 전부 오후 재료로 잡혀 창 분석이 오염된다. **지금은 관측 전용**: `source='naver'` 가 `NEWS_ACTIVE_SOURCES`(기본 `telegram`)에서 빠져 있어 라벨·룰·veto·LLM 판정에 일절 반영되지 않는다(적재 전/후 8개 조회 경로 스냅샷 동일 확인). 2~3주 병행 후 승격 — 승격 시 함께 할 일은 `sql/44` 주석에 있다(URL 제거 dedup·고유기사 기준 집계·rule 재등록·news_guard 편입 감사). 조회 실패는 종목 단위로 건너뛰고 계속하며, **403/429 만 사이클 조기 종료**(exit 1 로 job_run 에 남는다). 수동 실행 `--date`·`--limit`·`--dry-run` |
| ⏰ `news_ticker_seed` | 일 07:30 (등록 시 1회) | 키움 ka10099(코스피/코스닥) → `ticker_dictionary` ACTIVE 업서트. 뉴스 사전매칭 커버리지용 |
| ⏰ `cleanup_content` | 매일 04:00 | `content_analysis`·`content_skip` 3개월 + `news_mention` `NEWS_RETENTION_DAYS`(기본 **30**) 이전 행 삭제(테이블 비대화 방지). 14→30 상향(2026-07-31): 네이버 병행 검증이 "텔레그램만 vs 네이버 포함" 을 원자료에서 소급 재계산하는 방식이라 14일이면 검증 창이 앞에서 잘린다. 승격 판정 후 14로 되돌려도 된다 |
...
네이버 증권 종목별 뉴스(30분) ─매칭 X, 종목코드 조회─┘  source='naver' = 관측 전용
                                        │            (NEWS_ACTIVE_SOURCES 게이트 — 아래 소비 경로 전부에서 제외 중)
