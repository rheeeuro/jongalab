...
[cron] 매 거래일 17:50 (시간외단일가 세션 종료 직전). 단발 실행: uv run workers/after_hours_labels.py
"""
import logging
import time
...
def _stock_labels(api: KiwoomRestClient, code: str, today_dt: str) -> dict:
...
def run():
    ...
    logger.info(f"시간외·리스크 라벨 수집 시작 — 유니버스 {len(codes)}종목")
    rows = []
    for code in codes:
        base = code.split("_")[0]
        rows.append(_stock_labels(api, base, today_dt))
