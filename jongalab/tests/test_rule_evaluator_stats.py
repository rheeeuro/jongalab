...
특히 **유니버스 자기제외 초과 계열**(2026-07-28)을 고정한다. 이 계열이 selector 승격의
통계 게이트(ci_low_exc·t_days_exc)를 좌우하므로, 기준선 계산이 틀어지면 게이트가 조용히
잘못된 답을 낸다. 자기제외를 빼먹으면 rule 이 자기 자신을 기준선에 넣어 초과분이 0 쪽으로
