...
// label 은 그 자체로 뜻이 통해야 하고(약어·영문 단독 금지), short 는 타일 안에서 값 밑에
// 붙는 한 줄 힌트, help 는 '지표 설명' 패널에 들어가는 긴 설명이다. 셋 다 **중학생이 읽어서
// 이해되는 말**로 쓴다 — 표본·신뢰구간·유의성·회귀 같은 통계 용어를 쓰지 않는다.
export const STAT_META = {
  mean_net: { label: '평균 수익', short: '한 번에 평균 몇 %', help: '이 전략이 고른 종목을 다음 날 팔았다면 한 번에 평균 몇 % 벌었을지예요. 수수료와 세금(0.25%)은 이미 빼고 계산했어요.' },
  win_rate: { label: '성공률', short: '이긴 횟수의 비율', help: '번 횟수가 전체의 몇 %인지예요. 성공률이 높아도 크게 한 번 잃으면 소용없어서, 위의 평균 수익이 더 중요해요.' },
  ci_low: { label: '보수적 수익', short: '운 나빴을 때 기준', help: '운이 나빴다면 얼마나 벌었을지 낮춰 잡은 값이에요. 평균이 +2%라도 성적이 들쭉날쭉하면 이 값은 마이너스가 될 수 있어요. 이 값이 0보다 커야 "운이 아니라 진짜"로 보고 실제로 써요.' },
  worst_low_ret: { label: '최악 하락', short: '가장 나빴던 하루', help: '이 전략이 고른 종목이 다음 날 장중에 가장 많이 떨어졌던 폭이에요. 이 전략을 쓰면 겪을 수 있는 최악의 하루라고 보면 돼요.' },

  n_days: { label: '검증 거래일', short: '서로 다른 날이 며칠', help: '성적을 매긴 날이 며칠인지예요. 같은 날 걸린 종목 10개는 장이 좋으면 다 같이 오르니까 사실상 하루짜리 기록이에요. 그래서 서로 다른 날이 10일은 모여야 실제로 쓸지 검토해요.' },
  n: { label: '검증 횟수', short: '종목 수 × 날짜 수', help: '성적을 매겨 본 총 횟수예요. 하루에 3종목씩 10일이면 30번이에요. 이 숫자가 커도 며칠 안 되는 기록이면 믿기 어려워서, 판단은 옆의 검증 거래일로 해요.' },
  t_days: { label: '우연 아닐 점수', short: '1.65 넘어야 통과', help: '성적이 우연인지 아닌지를 점수로 나타낸 거예요. 하루를 한 판으로 세서 계산해요 — 같은 날 종목들은 장이 좋으면 다 같이 오르니까, 종목 수로 세면 실제보다 훨씬 대단해 보이거든요. 1.65보다 커야 우연이 아니라고 봐요(위험 회피 전략은 큰 사고를 막는 게 목적이라 이 조건에서 빼요).' },

  // ── 시장 회귀 계열. alpha 는 **내리는 판단(강등)에 실제로 쓰는 값**이고, 올리는 판단(승격)
  // 은 그대로 절대 평균 수익으로 한다. 초과 수익은 '장을 이겼나'라 시장을 덜 타는 방어형
  // 전략을 상승장에서 부당하게 깎는데, alpha 는 그 정도(beta)를 감안해서 뺀다.
  alpha: { label: '장 덕 뺀 수익', short: '알파 · 강등 판단 기준', help: '번 돈에서 "그날 장이 오르내린 몫"을 빼고 남은, 전략만의 성적이에요(알파라고 불러요). 장이 3% 오른 날 이 전략도 3% 벌었다면 전략의 몫은 0이에요. 반대로 장이 내린 날 안 잃었다면 전략의 몫이 있는 거고요. 지금 쓰고 있는 전략은 이 값이 마이너스로 돌면 내릴지 검토해요.' },
  beta: { label: '장 따라가는 정도', short: '1이면 장과 똑같이', help: '장이 1% 오를 때 이 전략이 몇 % 오르는지예요(베타라고 불러요). 1이면 장과 똑같이, 1.5면 장보다 1.5배 크게 움직여요. 0에 가까우면 장과 상관없이 움직인다는 뜻이에요. 이 값이 크면 오를 땐 많이 벌지만 내릴 땐 그만큼 크게 잃어요.' },
  t_alpha: { label: '위 수치 믿을 점수', short: '0 근처면 아직 몰라요', help: '바로 위 "장 덕 뺀 수익"을 얼마나 믿을 수 있는지 점수예요. 기록이 쌓인 날이 적으면 0 근처에 머물러요. 그럴 땐 위 숫자도 아직 참고만 하세요.' },
  down_day_mean: { label: '장 내린 날 성적', short: '장 빠진 날만 모아서', help: '장이 내린 날만 골라서 이 전략이 평균 몇 % 벌었는지예요. 플러스면 "장이 나빠도 안 잃는" 전략이라 가장 귀해요. 밑에 적힌 날 수가 적으면 아직 우연일 수 있어요.' },

  // ── 아래 초과 계열은 **적용 조건이 아니다**(2026-08-04). 상세 화면에서 "장이 좋아서 오른 건
  // 아닌지" 눈으로 확인하는 참고값이다 — 평균보다 덜 벌어도 안정적으로 수익이 나면 통과다.
  mean_exc: { label: '남들보다 번 몫', short: '같은 날 다른 종목 대비', help: '같은 날 다른 종목들의 평균보다 얼마나 더 벌었는지예요(이 전략이 고른 종목은 빼고 비교해요). 다만 장을 덜 따라가는 전략은 상승장에서 무조건 손해 보는 값이라, 실제 판단에는 쓰지 않고 눈으로만 확인해요.' },
  ci_low_exc: { label: '보수적으로 본 초과분', short: '운 나빴을 때 기준', help: '위 "남들보다 번 몫"도 운이 나빴을 경우로 낮춰 잡은 값이에요. 참고용이에요.' },
  t_days_exc: { label: '초과분 우연 점수', short: '참고용 점수', help: '"남들보다 번 몫"이 우연인지 아닌지를 점수로 나타낸 거예요. 참고용이에요.' },
} as const;

export type StatKey = keyof typeof STAT_META;

// ── 지표 묶음 — '누적 성적' 타일과 '지표 설명' 패널이 **같은 순서·같은 묶음**을 쓴다.
// 묶는 기준은 게이트가 실제로 던지는 질문 셋이다(돈을 버는가 / 우연이 아닌가 / 장 덕인가).
// 순서를 바꾸거나 지표를 추가할 땐 여기만 고치면 두 패널이 함께 따라온다.
export const STAT_GROUPS: {
  key: string;
  title: string;
  question: string;
  keys: StatKey[];
  reference?: boolean;   // 판정에 쓰지 않는 참고 묶음(흐리게 표시)
}[] = [
  {
    key: 'earnings',
    title: '얼마나 벌었나',
    question: '이 전략을 썼으면 돈이 됐을까?',
    keys: ['mean_net', 'win_rate', 'ci_low', 'worst_low_ret'],
  },
  {
    key: 'trust',
    title: '믿을 만한가',
    question: '기록이 충분히 쌓였고, 우연은 아닐까?',
    keys: ['n_days', 'n', 't_days'],
  },
  {
    key: 'market',
    title: '장 덕분인가, 전략 덕분인가',
    question: '장이 좋아서 번 건 아닐까?',
    keys: ['alpha', 'beta', 't_alpha', 'down_day_mean'],
  },
  {
    key: 'excess',
    title: '남들과 비교하면',
    question: '판단에는 쓰지 않고 눈으로만 확인해요.',
    keys: ['mean_exc', 'ci_low_exc', 't_days_exc'],
    reference: true,
  },
];
