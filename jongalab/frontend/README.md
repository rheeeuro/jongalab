...
| 탭 | 경로 | 역할 |
|---|---|---|
| 추천 | `/` | 가장 최근 리포트일의 추천 종목 전체 + 성적 요약 + 시장 요약(지수·섹터·뉴스) |
...
### `/` — 오늘의 추천 (홈·랜딩)
가장 최근 리포트일의 **선정 종목 전체** + 그 아래 시장 요약. 기준일은 '오늘'이 아니라 최신
리포트일이다(휴장일·장중에 화면이 통째로 비지 않게).
- `pick/PickHero` — 기준일 → h1 → "규칙 N개가 골랐습니다" → 접이식 면책.
...
- `home/HomeDigest` — 지수·주도 섹터·뉴스가 몰린 종목의 **압축 요약** 3종(각 한 줄 가로 스크롤).
  ⚠️ 여기에 카드·차트·필터를 붙이지 말 것 — 홈이 다시 종합 대시보드가 되면 추천이 묻힌다.
  한 줄을 넘어서는 내용은 해당 탭(`/market`·`/news`)에 둔다. 지수는 외부 API 라 `Suspense` 스트리밍.

### 픽 카드 (`StockReportCard`) — 홈·`/reports/{date}` 공유
- **표시 순번(1,2,3…)을 매기지 않는다.** hybrid/rules 모드의 선정은 점수 순위와 무관해
  목록 순번을 매기면 점수로 뽑힌 것처럼 읽힌다. 순위 대신 **점수 배지 + '왜 뽑혔나'** 를 낸다.
- '왜 뽑혔나'에는 룰의 **한글 제목**(`edge_rule.title`)을 최대 2개 + `외 N개 규칙`.
