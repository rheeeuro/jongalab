...
/**
 * 헤드라인 스트림 — 그 날 수집된 기사를 최신순으로 흘려 준다 (2026-08-03 신설).
 *
 * 예전 뉴스 탭에는 **헤드라인이 한 줄도 없었다** — "하루 수백 건 파이어호스는 읽히지 않는다"는
 * 이유로 의도적으로 뺐던 것인데, 그 결과 뉴스 탭에서 오늘 무슨 기사가 났는지는 볼 수가 없고
 * 종목 상세로 들어가야만 했다. 지금은 세 가지로 파이어호스 문제를 막는다:
 *   1) **시세 기사를 기본으로 숨긴다** — "급등/상한가/특징주" 류 실측 21%. 판별은 후속 재료
 *      채점과 같은 함수(`is_price_report`)라 화면과 채점의 기준이 갈리지 않는다.
 *   2) **큐레이션 계층 아래에 둔다** — 요약·재료 목록을 먼저 보고 원하면 원문으로 내려간다.
 *   3) **페이지당 40건씩만** 받고 나머지는 '더 보기'로 미룬다(그 날 1,000건대다).
 *
 * 소스는 2026-08-05 부터 **네이버 증권 섹션 기사**(`sec_news`)다. 그 전엔 텔레그램 종합 속보
 * 채널을 읽어서 `70대 남성 사망`(남성 004270)·`한화, 삼성 4-1 제압`(한화 000880) 같은
 * 주식과 무관한 기사가 4.1% 섞여 있었다 — 모집단이 원인이라 모집단을 바꿨다.
 * 종목 칩(`stocks`)은 없는 기사가 많다(실측 64%) — 시황·환율·정책 기사가 원래 그렇다.
 *
