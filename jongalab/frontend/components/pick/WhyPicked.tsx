 ...
 */
export function WhyPicked({ report: r }: { report: StockReport }) {
  const axes = [
    {
      icon: Activity,
      label: "수급",
    ...
    <section className="rounded-3xl bg-white p-5 sm:p-6 dark:bg-slate-900/60">
      ...
      <div className="mt-4 space-y-2.5">
        {axes.map(({ icon: Icon, label, badge, note, text }) => (
          <div
            key={label}
            className="flex items-start gap-3 rounded-2xl bg-slate-50 px-3.5 py-3 dark:bg-slate-800/40"
          >
            <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white text-slate-500 dark:bg-slate-900 dark:text-slate-400">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-sm leading-relaxed font-bold text-slate-800 dark:text-slate-100">
                {text}
              </p>
            </div>
          </div>
