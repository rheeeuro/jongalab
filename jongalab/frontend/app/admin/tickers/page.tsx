  ...
  const handleLoadSector = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setEditError(data?.detail || data?.error || "섹터 조회에 실패했습니다.");
        return;
      }
      const data = await res.json();
      const loaded: string | null = data?.sector ?? null;
      if (loaded) {
        setEditForm((f) => ({ ...f, sector: loaded }));
      } else {
        setEditError("외부 API에서 섹터 정보를 찾지 못했습니다.");
      }
    } catch (e) {
            ...
            <div className="space-y-4 py-2">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-slate-700 dark:text-slate-200 min-h-[38px] flex items-center">
                    {editForm.sector ? (
                      <span className="font-medium">{editForm.sector}</span>
                    ) : (
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="outline"
                    onClick={handleLoadSector}
                    disabled={sectorLoading || !editForm.ticker_symbol.trim()}
                  >
                    <RefreshCw
                      className={`w-3.5 h-3.5 mr-1 ${sectorLoading ? "animate-spin" : ""}`}
                    />
                    {sectorLoading ? "조회 중" : "섹터 로드"}
                  </Button>
                </div>
