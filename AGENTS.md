...
5. 순수 로직 단위 테스트 실행(DB/네트워크 불필요). 동작이 바뀌면 해당 `tests/` 도 함께 갱신:
   - `trading/` 자금 경로(`seed_allocator`/`execution_engine`/`risk_engine`/`watchdog`) → `uv run --directory trading --group dev pytest`
   - `jongalab/` 가중치 백테스트(`core/backtest.py`) → `uv run --directory jongalab --group dev pytest`
     (`recompute_score` 는 실제 `score_candidate` 와 교차검증 — 엔진 공식 변경 시 미러도 갱신)
