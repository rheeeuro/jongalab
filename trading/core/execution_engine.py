...
class ExecutionEngine:
    ...
    def execute_buy(self, trade_date: str, signal: dict, dmst_stex_tp: str = "KRX") -> dict | None:
        ...
        if not paper and (resp.get("return_code") != 0 or not resp.get("ord_no")):
            ...
            # 증거금 부족 거부면 키움이 안내한 '매수가능 N주'로 1회만 축소 재시도
            afford = self._afford_qty_on_margin_reject(resp)
            if 1 <= afford < qty:
                return self._retry_buy_reduced(trade_date, signal, key, afford, price, dmst_stex_tp)
            return None
    ...
    @staticmethod
    def _afford_qty_on_margin_reject(resp: dict) -> int:
        """증거금 부족 거부 응답에서 키움이 안내한 '매수가능 N주'를 파싱. 해당 없으면 0.
        예: '[2000](855056:매수증거금이 부족합니다. 2주 매수가능)' → 2
        """
        msg = resp.get("return_msg") or ""
        if "증거금" not in msg:
            return 0
        m = re.search(r"(\d+)\s*주\s*매수가능", msg)
        return int(m.group(1)) if m else 0

    def _retry_buy_reduced(self, trade_date: str, signal: dict, base_key: str,
                           qty: int, price: int, dmst_stex_tp: str) -> dict | None:
        """증거금 부족 거부 후, 키움이 안내한 매수가능 수량으로 1회만 축소 재시도(live 전용).
        새 멱등키(base_key+':retry')로 기록해 원거부와 구분하고 중복 재시도를 막는다.
        """
        stk_cd = signal["stk_cd"]
        key = f"{base_key}:retry"
        if order_repo.find_by_idempotency_key(key):
            return None
        decision = self.risk.check(trade_date, stk_cd, qty * price)
        if not decision.allowed:
            audit_log.append("buy_blocked", stk_cd, {"key": key, "reason": decision.reason})
            logger.warning("축소 재시도 차단 [%s]: %s", stk_cd, decision.reason)
            return None
        order_id = order_repo.create_intended(key, signal["id"], stk_cd, "buy", qty, price, "market", "live")
        audit_log.append("buy_retry_intended", stk_cd, {"order_id": order_id, "qty": qty, "price": price})
        resp = self.client.buy(stk_cd, qty, 0, trde_tp=self._now_trde_tp(dmst_stex_tp),
                               dmst_stex_tp=dmst_stex_tp)
        audit_log.append("buy_response", stk_cd, {"order_id": order_id, "resp": resp})
        if resp.get("return_code") != 0 or not resp.get("ord_no"):
            order_repo.mark_sent(order_id, None, "rejected")
            audit_log.append("buy_rejected", stk_cd, {"order_id": order_id, "resp": resp})
            logger.warning("축소 재시도도 거부 [%s] %d주: %s", stk_cd, qty, resp.get("return_msg"))
            return None
        order_repo.mark_sent(order_id, resp.get("ord_no"), "sent")
        self.risk.record_order(trade_date)
        logger.info("증거금 부족 → %d주로 축소 재시도 성공 [%s]", qty, stk_cd)
        return resp
