...
⚠️ **`jongalab/core/edge_predicate.py` 의 최소 복제다**(2026-08-03). 두 서브프로젝트는 venv 가
분리돼 서로 import 할 수 없어, kiwoom 이 `core/{config,db,logging_setup}.py` 를 복제하는 것과
같은 패턴으로 가져왔다. **원본을 고치면 이 파일도 같이 고칠 것** — 채점(jongalab rule_evaluator)과
집행(trading signal_executor)이 같은 predicate 를 다르게 해석하면 "측정한 것과 다른 것을 산다".
드리프트는 `tests/test_edge_predicate_copy.py` 가 계약(op 목록·NULL 규약·AND 결합)으로 잡는다.
...
DB·네트워크 무의존이므로 tests/test_edge_predicate_copy.py 단위 테스트로 계약을 고정한다.
"""
