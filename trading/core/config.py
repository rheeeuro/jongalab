...
키움 주문 인증은 공유 DB(kiwoom_token)의 access_token 을 쓴다 — 이 모듈에 키가 없다
(kiwoom_order_client.ensure_token). APP_KEY/SECRET_KEY 는 kiwoom 서버 전용이다.
"""
import os
...
# 거래소 라우팅(KRX/NXT)은 설정이 아니라 호출부가 정한다 — 매수는 workers/signal_executor.py
# 의 VENUES(창별 거래소), 매도는 workers/monitor.py 의 resolve_sell_venue(시각별)와
# workers/settle.py 의 단계별 지정을 따른다.
