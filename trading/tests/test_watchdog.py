...
def test_all_done_no_alert():
    done = {"settle:nxt", "settle:krx", "monitor"}
    assert missing_workers((9, 35), done, CRIT) == []


def test_all_missing_after_deadlines_all_reported():
    assert _names(missing_workers((9, 35), set(), CRIT)) == {"settle:nxt", "settle:krx", "monitor"}


def test_only_overdue_workers_flagged():
    # 08:30 시점 — settle:nxt(08:05)만 마감 지남. settle:krx/monitor 는 아직 마감 전.
