...
│   ├── seed_allocator.py       # 시드 배분(거래소별): 상위10 선정(점수순)·**확신도(선정 근거 수) 비례 배분**(근거 없으면 등가중, .env SEED_CONVICTION_MAX_MULT 상한 3표)·표 대비 최소투입 우선 그리디·종목당 시드 25% 캡(고가주 첫 1주만 캡×2 이내 예외 — 2026-07-10 HLB 하한가 사건으로 50%→25%, .env SEED_MAX_NAME_PCT)
...
│   ├── ex_rights.py            # 권리락 스킵 조회: jongalab ex_rights_schedule(sql/48) 읽기 전용 — **다음 거래일**(=이 매수분의 청산일)이 권리락일인 종목은 signal_executor 가 매수하지 않는다(기준가가 배정비율만큼 기계적으로 낮아지고 공짜 신주는 3주 뒤에 들어와 그 자리에서 상계되지 않는다. 2026-08-04 알테오젠: 종가 346,500→익일 기준가 267,000). 평단·손절선은 건드리지 않는다(가격 보정이 아니라 회피). 조회 실패·캘린더 비어 있음 → 미개입(매수 정상 진행)
...
signal_executor (KRX 15:00 / NXT 19:30)
  · 블록리스트 제외 → **권리락 제외**(다음 거래일이 권리락일인 종목은 skip, note=`ex_rights`, `core/ex_rights.py`) → 거래소 분류 → 시드 산정 → **최초 시드 배율(risk_config `SEED_INIT_MULT`, 게이트보다 먼저)** → **regime_gate(역전 레짐)로 총 시드 축소 — 2026-07-23 기본 OFF(미개입 1.0), audit 관찰만** → **확신도(선정 근거 수) 산정** → seed_allocator 표 비례 배분 → **futures_gate(선물 하락 시 섹터별 수량 감액, KRX·NXT) × macro_gate(보유 창 sev3 예정 이벤트 시 공통 감액)** → 데드라인(15:20/19:50) 종가 매수. NXT 는 그 직전에 **집행 레이어 rule**(원장 live rule × 실시간 야간갭, risk_config `NXT_GAP_FILTER_ENABLED`)이 룰 선정분의 매수 여부를 한 번 더 가른다
...
monitor (08:00~09:30, 실시간 틱 판정 + 15초 유지보수 — 2026-07-31 WS 전환)
  ...
  · 하트비트(`monitor_poll`)에 `ws` 상태(연결·틱수·**수급틱수**·재연결수·마지막 틱 나이)와 `cooldown_skips` 를
    담아 대시보드에서 피드 가동 여부를 확인한다. 손절/스탑/베토 audit 에는 `path`(tick|slow)와
    `slow_wait_ms`(15초 폴링이었다면 더 기다렸을 시간)를 남겨 **즉시 판정의 실이득을 사후 채점**한다.
  · **실시간 수급 관측(2026-08-03, `SUPPLY_FEED_ENABLED`) — 수집 전용, 판정 미사용**:
    매도 발동 payload 에 그 시점 수급 스냅샷(`supply`)을 붙이고, 매도 안 한 종목의 **대조군**으로
    `SUPPLY_LOG_SEC`(60초) 주기 `monitor_supply` 행을 남긴다(평단·현재가 포함).
    수집 항목 — 0B 틱에 이미 실려 오는 **체결강도(228)·매도/매수 체결량(1030·1031)·매수비율(1032)**
    (추가 구독 0) + 별도 그룹으로 구독하는 **0w 프로그램 매도/매수·순매수(202·206·210·212)**.
    필드 의미는 산술 교차검증으로 확정(체결강도 == 1031/1030×100, 순매수 == 206−202).
    왜 관측부터인가: 이 축은 **과거 데이터가 없다** — ka90008 은 `date` 를 줘도 무시하고 당일치만
    반환하고(7/31·7/24·7/10 모두 당일 응답), 실시간 수급을 저장한 적도 없어 백테스트가 불가능하다.
    프리마켓 존재는 확인됨(오늘 042660 08:00:01~ 프로그램 시계열 1,131틱, 전부 non-zero) —
    WS 0w 가 08시대에도 틱을 주는지는 이 수집이 스스로 답한다(안 오면 ka90008 REST 1분 폴링으로 대체:
    1페이지 200틱/0.36초). 검증 후 유망 방향은 **손절 강화 쪽**(수급 악화 시 −2% 전에 청산, reduce-only).
    ⚠️ 값이 없거나 낡아도, 관측이 통째로 예외로 실패해도 손절·스탑·트레일링은 완전히 동일하게 동작한다.
...
| 확신도 사이징(선정 근거 수) | `seed_allocator.py`(`conviction_from_signal`), `signal_executor.py`, `/buy-preview` + `.env` `SEED_CONVICTION_MAX_MULT`(기본 3.0) | 2026-08-03 도입. **등가중의 단위를 '종목'에서 '선정 근거 1표'로 바꾼 것**이다: 표 = 매칭 selector rule 수(`trade_signal.rule_names`) + **legacy 점수 1표**(그날 점수 top-N 에도 들었으면 — 판정 N 은 `edge_rule.get_selected_count`, 집행 레이어 rule 의 `in_scope` 와 **같은 기준**). 목표금액 = `seed × 표/Σ표`, 잔여 그리디도 **표 대비 투입액(cost/w)** 이 적은 종목부터 채운다. 표 없음/조회 실패 → 전원 1표 = **종전 등가중과 완전 동일**(보수적 폴백). ⚠️ **점수 '크기' tilt 는 여전히 금지** — 점수는 익일 손익을 예측하지 못했고(역상관), 여기서 쓰는 건 서로 다른 근거의 **중복 매칭 개수**뿐이다. 종목당 시드 25% 캡(`SEED_MAX_NAME_PCT`)은 확신도와 무관하게 그대로 적용된다(하한가 1방 봉쇄는 캡만이 한다). ⚠️ **미검증 통설**("근거가 많으면 기대값이 높다") — 매 배분을 `audit_log('seed_conviction')`(표 수·rule_names·rank_no·수량) + `buy_exec.conviction` 으로 남겨 사후 채점한다. 롤백은 `.env` `SEED_CONVICTION_MAX_MULT=1.0`(코드 수정 불필요). `/buy-preview` 도 동일 계산으로 표시(모니터 탭에 "근거 N개 · 비중 증액") |
...
| 집행 레이어 rule (NXT 야간갭) | `edge_execution.py`, `edge_predicate.py`, `repository/edge_rule.py`, `signal_executor.py` + risk_config `NXT_GAP_FILTER_ENABLED` | **NXT(19:50) 전용 · 원장 기반.** 19:50 주문 직전 갭(`(NXT 현재가 − KRX 확정 종가)/KRX 종가`)을 계산해 **jongalab 원장의 live rule predicate 에 먹여** 매수 여부를 정한다 — 밴드·조건은 rule 이 갖고(하드코딩 상수 아님) 승격/강등이 실제로 필터를 켜고 끈다. 2026-08-03 `f3_nxt_gap_quality` live(갭 1~6% + NXT상장 + 섹터동조 + 등락률 0~12%). **왜 집행 레이어인가**: 갭엔 NXT 시세가 필요한데 매수는 19:50 데드라인 단일 주문이고 신호 핸드오프(19:30)·시드 배분은 그보다 앞선다 → 원리상 주문 직전에만 알 수 있어 선정 레이어(`edge_selection`)에선 영원히 NULL→무음이다. 그래서 jongalab `edge_policy.EXECUTION_TIME_COLS`/`rule_layer` 가 이 레이어를 인정하고, 집행기가 원장을 읽어(`get_jongalab_db`, regime/macro 게이트와 같은 패턴) predicate 를 평가한다. 채점(rule_evaluator)이 쓰는 `nxt_gap_pct`(gap_check 19:50)와 **같은 정의**라 rule 의 과거 stats 가 그대로 승격 근거가 된다. **두 가지 제약**: ① **종목 추가 불가** — 후보 풀·시드가 19:30 에 확정된 뒤라 화이트리스트로만 동작하고 거른 몫의 시드가 **논다**(하이브리드 대체 없음). ② **적용 대상은 그 rule 이 혼자 데려온 종목뿐**(`edge_execution.in_scope`, 2026-08-03 사용자 결정) — **다른 rule 과 함께 선정**(그 rule 이 별도 근거를 갖는다) · **점수 top-N 에도 포함**(룰 없이도 선정될 종목이었다) · **점수순으로만 선정**(하이브리드의 '잔여 슬롯은 점수순'을 집행이 뒤집지 않는다) 은 전부 비대상이다. 즉 "이 rule 때문에 들어온 종목만 이 rule 로 재확인한다". 점수 top-N 판정의 N 은 그날 선정 종목 수로 근사한다(hybrid 총 상한이 top_n 이므로 일치. `repository/edge_rule.get_selected_count`). rank_no·N 을 모르면 비대상(보수적). **fail-open 4종**: 집행 레이어 live rule 없음 / 점수 선정분 / 갭 판정 불가 / 리포트 행 없음 → 전부 매수. 오설정 rule 하나가 전 종목을 막지 않는다(그 rule 만 무시). 감액 게이트의 미개입 폴백과 같은 규약이다. KRX 종가(갭 분모)·리포트 행·live rule 은 **데드라인 전** 윈도우 대기 중에 미리 확보한다 — 19:50 루프에 DB·API 호출을 더하면 주문이 늦어지고, 분모는 ka10081 당일 캔들로 고정한다(ka10001 무접미사 `cur_prc` 는 야간에 KRX 종가인지 NXT 시세인지 일관되지 않다: 실측 19:50 엔 KRX 종가, 20:00+ 엔 NXT 가격). **전건 `audit_log('nxt_gap_filter')`** 기록(갭·매칭 rule·in_scope·사유) — 비대상까지 남겨야 "범위를 넓혔다면?"이 반사실로 계산된다. ⚠️ **기대효과 미측정**: 세션 실측 +0.557%p(t=1.12, 16거래일)는 선정된 NXT **전체**를 거른 결과이고, 현행 범위(단독 선정분)는 표본이 훨씬 좁아 소급 측정이 불가하다. 근거는 '가장 보수적'이라는 것뿐 — 실제 성적은 live rule 채점·강등 감시가 판정한다. **선정 레이어와 짝**: jongalab closing_bet 이 저녁 회차(15:40+)에 `{code}_NX` 현재가로 그 시점 갭을 재서 룰 평가 dict 에 넣으므로(DB `nxt_gap_pct` 는 19:50 패스 소관 — 저장하지 않는다) 이 rule 이 선정에서 종목을 데려올 수 있고, 종목 탭이 `rule_names` 로 근거를 보여준다. 즉 선정은 그 회차 갭(19:30), 최종 매수 판정은 집행기의 19:50 갭이다. 정식 검증은 원장의 rule 채점·강등 감시가 담당. 롤백: 토글 0(또는 rule 강등) |
...
| 죽은 주문 판정 가드 | `order_maintenance.reconcile_dead_sent`, `order.get_open_sent_aged` + `DEAD_ORDER_MIN_AGE_SEC`(60초) | 0주 체결로 소멸한 IOC 를 `canceled`+멱등키 해제(`:dead:<id>`)로 마감해 재매도를 열어주는 장치인데, 판정 기준('브로커 미체결에 없음 + 체결 0')은 소멸과 **전량체결 직후**를 구분하지 못한다 — 체결된 주문도 미체결(ka10075) 목록엔 없고 체결내역(ka10076) 반영은 몇 초 늦는다. 그래서 **① 전송 후 60초 미경과 주문은 판정 보류**(SQL 단계에서 제외 → 폴링당 API 호출도 안 늘어난다) **② 브로커 체결내역에 체결수량이 있으면 보존**(직전 `sync_fills` 가 예외로 실패해도 오판정을 막는 2중 가드) ③ 미체결로 살아있음 ④ 로컬 체결분 존재 — 4중 가드. 대가는 재매도가 최대 60초 늦어지는 것뿐이고(그 사이 하드손절·스탑 판정은 정상 진행) 체결을 잃는 쪽이 훨씬 비싸다. 계기: 2026-08-05 레인보우로보틱스 — 08:03:12 전량체결된 매도를 1초 뒤 폴링이 canceled 로 마감 → `sync_fills`(status='sent' 만 조회)가 다시는 못 봐 체결·실현손익(+7,973원)이 누락되고 **유령 포지션 1주**가 남아 대시보드에 "안 팔림"으로 표시(09:28 데드라인 백스톱이 없는 주식에 매도 주문을 낼 상태였다). 오판정 복구는 `order.unvoid_dead_order(id)` → 'sent' 복귀 + 멱등키 원복 후 `sync_fills` 가 정상 경로로 반영한다(값을 손으로 써넣지 않는다) |
| 정합성 점검 | `reconcile.py` | 매일 브로커 잔고 vs 로컬 포지션 대조 — **감지·알림만 하고 자동 교정은 하지 않는다**(drift 는 `reconcile_drift` audit + 텔레그램) |
...
`SUPPLY_FEED_ENABLED`/`SUPPLY_LOG_SEC`(실시간 수급 관측 — 수집 전용이라 끄고 켜도 매매 동작 불변).
...
**윈도우 종료 표시** (`/buy-preview` 응답 `venues[].closed`, 2026-08-05): 데드라인(KRX 15:20 / NXT 19:50)이
지난 거래소는 `closed=true` 로 내려보내고 **시드·수량 0 · 종목 note=`윈도우 종료`**(배분·선물게이트 조회도 건너뜀),
카드에선 시드 자리에 "윈도우 종료 — 오늘 집행 안 됨"을 띄운다. 계기: 2026-08-05 GS건설(006360) — **NXT 미상장
= KRX 전용**인데 16:00 closing_bet 회차에 후보로 새로 올라와 15:20 KRX 데드라인을 이미 넘겼다. 신호는 pending 이라
프리뷰가 계속 "KRX 매수 예정"으로 보여줬지만 KRX 윈도우는 다시 열리지 않고 19:30 NXT 회차는
`is_nxt == want_nxt` 에서 걸러내므로 **그날 매수 경로가 없는 신호**였다. 표시 계층만 바꾼 것 —
거래소별 시드는 여전히 `전체 점수합`으로 나누므로(executor 와 동일) 남은 거래소 배분액은 무변경이다.
...
(fake 협력 객체 주입 + repository 함수 monkeypatch). 커버리지: 시드 배분(`seed_allocator` — 캡·첫1주 예외 + **확신도 표 비례·클램프·표 없으면 등가중 동일·표 계산법**),
...
집행 레이어 rule 판정 레이어 감지·fail-open 4종·점수 선정분 불개입·오설정 rule 격리·행 비변형·밴드는 predicate 소관(`test_edge_execution`), NXT 갭 산식·재료 부재 시 None(`test_nxt_gap_filter`), predicate 평가기 복제 드리프트(`test_edge_predicate_copy` — jongalab 원본과 op·NULL 규약·AND 결합이 어긋나면 '측정한 것과 다른 것을 산다'),
레버리지 ETF 치환 순수 로직(무매핑 무치환·코드/이름만 교체·원신호 id 유지·ETF 코드 결측 방어, `test_leverage_swap`),
죽은 주문 판정 가드(`test_order_maintenance` — 경과시간 미달이면 브로커 조회조차 안 함·브로커 체결 있으면 보존·체결수량 0 row 는 보호 아님·진짜 소멸분은 계속 정리·조회 실패 시 보류),
...
**수급 관측이 판정에 개입하지 않음**(0B 부가 필드 파싱, 0w 는 가격·틱이벤트 불변, 토글 off 시 미수집,
`get_supply` 예외 시 `_supply_snapshot`→{} 로 매도 경로 정상).
