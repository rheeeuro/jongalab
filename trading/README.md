...
**청산 종목 워커 로그**: 홈·캘린더의 청산 목록(`RoundTrips`)에서 종목을 누르면 그 종목의
워커 활동 트레일(매수 집행 → 갭/스탑 모니터 → 매도 체결)을 모달로 보여준다. `GET /stock-events`
(`audit_log.list_by_stock`, 매수일~매도일 구간, 하트비트·주문응답 원문 제외)을 조회하고,
