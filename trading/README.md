...
| 집행 레이어 rule | `edge_execution.py`, `signal_executor.py` + risk_config `NXT_GAP_FILTER_ENABLED` | **NXT(19:50) 전용 · 원장 기반.** 주문 직전 갭을 계산해 **jongalab 원장의 live rule predicate 에 먹여** 매수 여부를 정한다(밴드는 rule 소관, 하드코딩 상수 아님). **제약 2**: ① 종목 추가 불가(화이트리스트로만 동작, 거른 몫의 시드가 논다) ② **적용 대상은 그 rule 이 혼자 데려온 종목뿐**(`in_scope` — 다른 rule 과 함께 선정·점수 top-N 포함·점수순 선정은 비대상). **fail-open 5종**(rule 없음/점수 선정분/갭 판정 불가/리포트 행 없음/**시황 축 요구**)이라 오설정 rule 하나가 전 종목을 막지 않는다. 시황 축(`market.*`)을 쓰는 rule 은 집행기가 **판정하지 않고 매수로 흘린다** — 시장 스냅샷을 갖지 않으므로 평가하면 NULL 매칭 실패가 되고, 이 레이어는 미매칭이 곧 매수 스킵이라 조용히 전건 거부된다. 그 축은 jongalab 선정 회차(19:40)가 `market_snapshot` 슬롯 1935 로 평가하는 몫이다. KRX 종가(갭 분모)·리포트 행·live rule 은 **데드라인 전** 윈도우 대기 중에 미리 확보하고, 분모는 ka10081 당일 캔들로 고정한다. 전건 `audit_log('nxt_gap_filter')` |
...
**0주 사유·감액 이유 표시**(`/buy-preview` 응답 `stocks[].note`·`keep_reason`): note 는
`게이트 감액`(**게이트 전 배분이 1주 이상이었는데 keep 으로 0주가 된 경우만**) / `배분 0주(시드 부족)`
(단가 > 배분액 — keep<1 이어도 게이트 탓이 아니다) / `현재가 없음` / `윈도우 종료` 로 갈린다.
`keep_reason`(keep<1.0 일 때만)은 감액의 출처를 한 줄로 — **하락 축별 등락·강도**(코스피 선물/NQ/
미 프리마켓 반도체·한국물) + **섹터 클래스·keep** + **반올림 후 실제 수량 변화**. 홈 카드는 이 줄을
**목록 행 hover(`title`)로만** 보여준다 — 목록 스타일을 그대로 두려고 별도 표시 줄은 두지 않았다
(`app/page.tsx`).
...
- 집행 레이어 rule — 레이어 감지·fail-open 5종(시황 축 요구 포함)·점수 선정분 불개입·오설정 rule 격리·행 비변형·
