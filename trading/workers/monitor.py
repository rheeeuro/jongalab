...
30초마다 모든 보유 포지션을 점검:
  1) 하드 손절(칼손절): 현재가가 평단(avg_price) 대비 HARD_STOP_LOSS_PCT% 아래면
     settle_plan 유무와 무관하게 **즉시 전량 매도**. 정각(08:00/09:00)~settle(:05) 사이
     갭하락으로 손실이 커지기 전에 끊는 안전망.
  2) 스탑/저가이탈: settle(NXT 08:05)가 기록한 활성 청산계획(settle_plan)의 stop_price
     이하로 내려가면 **즉시 잔량 전량 매도**.

가동 구간은 평일 08:00~09:30 (NXT 개장 ~ KRX 잔량청산 + 여유)으로 한정한다.
08:00 정각부터 감시해 settle(08:05) 이전 구간도 하드 손절로 보호한다. KRX 청산(settle
09:05)이 갭 방향 무관하게 모든 잔량을 정리하므로 그 이후엔 stop_price 감시할 plan 이 없다.

pm2 cron 워커(autorestart:false, cron_restart '0 8 * * 1-5'): 08:00 에 기동돼 가동 구간이
...
def main() -> int:
    if not in_window(datetime.now()):
        ...
        return 0
    logger.info("하드손절(-%.1f%%)/스탑 모니터 시작 (30초 폴링, 08:00 기동, 09:30 자동 종료)",
                HARD_STOP_LOSS_PCT)
