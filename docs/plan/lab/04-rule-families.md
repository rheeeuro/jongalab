...
### `f5_frgn_surge_pullback` · `f5_frgn_surge_carry` (2026-07-19 추가 — sql/23, 눌림/지속 대칭 쌍)
- **출처**: 종가베팅 팁 PDF(트레이더 칼럼) 통설 "외인 대량 유입 후 수급 1음봉/2음봉에서 종가
  분할 매수" — 통설의 양의 가설(눌림)과 그 대칭(지속)을 한 축으로 등록해 데이터가 가른다.
...
- **predicate**: 눌림 `days_since_frgn_surge between [1,2]` AND `red_candle == 1` /
  지속 `days_since_frgn_surge between [1,2]` AND `red_candle == 0` AND `change_pct >= 0`
- **role**: 눌림은 **benchmark** — 음봉 표본 대부분이 음전이라 live selector 로는 무의미
  (선정 레이어가 음전 후보를 핸드오프에서 제외). 음(−) 확정 시 veto 재등록 후보,
  양(+) 확정 시 집행 설계 재검토. 지속은 selector(전 컬럼 선정 시점 계산, live 승격 가능).
- **사전 점검(2026-07-19, 6/26~7/18 인샘플 재현, 참고용)**: 기준선 +0.18%(n=345).
  서지 후 1~2일 전체 n=140 **+0.61%**(t=1.63) / 눌림 n=77 **−0.18%**(승률 44.2% — 통설과
  달리 약음) / 지속 n=61 **+1.47%**(승률 67.2%, t=2.03). 축이 인샘플에서 이미 갈린다 —
  out-of-sample 이 판정. 기관 동반 조건(PDF의 "기관 연속성")은 인샘플에서 개선 효과가
  없어 별도 rule 로 등록하지 않았다.
